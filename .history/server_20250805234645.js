    ...
    for (let i = 0; i < user.tasks.length; i++) {
        ...
        if (!task.completed) {
            ...
            if (task.progress >= task.target && !task.completed) {
                ...
                // POZI miktarı * çarpım kazanca ekle
                if (task.requiredPayment > 0) {
                    // Ödeme gerektiren görevlerde POZI miktarı * çarpım ekle
                    const poziReward = user.poziAmount * (task.requiredPayment / user.poziAmount);
                    user.balance += poziReward;
                    reward += poziReward;
                    console.log(`${user.username} kullanıcısı ${task.id}. görev için ${poziReward} POZI kazandı (POZI miktarı: ${user.poziAmount}, Çarpım: ${task.requiredPayment / user.poziAmount})`);
                } else {
                    // Normal görevlerde bakiyeyi 1.5 katına çıkar
                    const oldBalance = user.balance;
                    user.balance *= 1.5;
                    reward += user.balance - oldBalance;
                }
