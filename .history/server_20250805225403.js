    ...
    user.tasks.forEach(task => {
        if (!task.completed) {
            ...
            if (task.progress >= task.target && !task.completed) {
                // Eğer görev ödeme gerektiriyorsa ve kullanıcı ödeme yapmamışsa POZI miktarından düş
                if (task.requiredPayment > 0 && !user.hasPaid) {
                    if (user.poziAmount >= task.requiredPayment) {
                        user.poziAmount -= task.requiredPayment;
                        console.log(`${user.username} kullanıcısı ${task.requiredPayment} POZI ödeme yaptı (POZI miktarından düşüldü)`);
                    }
                }
