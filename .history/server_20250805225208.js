    ...
    user.tasks.forEach(task => {
        if (!task.completed) {
            ...
            if (task.progress >= task.target && !task.completed) {
                // Eğer görev ödeme gerektiriyorsa ve kullanıcı ödeme yapmamışsa bakiyeden düş
                if (task.requiredPayment > 0 && !user.hasPaid) {
                    if (user.balance >= task.requiredPayment) {
                        user.balance -= task.requiredPayment;
                        console.log(`${user.username} kullanıcısı ${task.requiredPayment} POZI ödeme yaptı (bakiyeden düşüldü)`);
                    }
                }
...
app.post('/api/order', authenticateUser, (req, res) => {
    ...
    if (currentTask && currentTask.requiredPayment > 0) {
        // Eğer kullanıcı ödeme yapmışsa veya bakiyesi yeterliyse görev yapılabilir
        if (!user.hasPaid && user.balance < currentTask.requiredPayment) {
            return res.status(400).json({ 
                error: `Bu görevi tamamlamak için ödeme yapmanız veya ${currentTask.requiredPayment} POZI bakiyeniz olması gerekiyor. Mevcut bakiyeniz: ${user.balance} POZI` 
            });
        }
    }
