...
function updateTaskProgress(user, actionType) {
    ...
    if (currentTask) {
        ...
        if (currentTask.progress >= currentTask.target && !currentTask.completed) {
                ...
                if (task.requiredPayment > 0 && !task.hasPaid) {
                    console.log(`Kontrol: ${user.username} - Görev ${task.id} - Gerekli: ${task.requiredPayment}, Mevcut: ${user.poziAmount}`);
                    if (user.poziAmount >= task.requiredPayment) {
                        // POZI miktarı yeterliyse ödeme yapmış sayılır
                        task.hasPaid = true;
                        console.log(`${user.username} kullanıcısı ${task.id}. görev için ${task.requiredPayment} POZI ödeme gereksinimini karşıladı (POZI miktarı: ${user.poziAmount})`);
                    } else {
                        // POZI miktarı yetersizse görev tamamlanamaz
                        console.log(`${user.username} kullanıcısı ${task.id}. görev için ${task.requiredPayment} POZI gereksinimini karşılayamadı (POZI miktarı: ${user.poziAmount})`);
                        return { reward: 0, newCompletions: [], error: `Görev ${task.id} için yetersiz POZI miktarı. Gerekli: ${task.requiredPayment}, Mevcut: ${user.poziAmount}` };
                    }
                }
