...
app.post('/api/admin/set-pozi-amount', authenticateUser, (req, res) => {
    ...
    if (!user.tasks || user.tasks.length === 0) {
        ...
        // Mevcut görevleri koru, sadece requiredPayment değerlerini güncelle
        user.tasks.forEach(task => {
            task.requiredPayment = calculateRequiredPayment(task.id, user.poziAmount);
        });
        console.log(`POZI miktarı güncellendi: ${user.username} - ${oldPoziAmount} → ${poziAmount} POZI`);
        console.log(`Mevcut görevler korundu, ödeme miktarları güncellendi`);
    }
