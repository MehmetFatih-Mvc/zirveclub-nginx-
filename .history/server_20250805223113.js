...
function generateTasksForUser(poziAmount, paymentCount = 0) {
    return baseTaskDefinitions.map(task => ({
        ...task,
        progress: 0,
        completed: false,
        completedAt: null,
        requiredPayment: calculateRequiredPayment(task.id, poziAmount, paymentCount)
    }));
}

        function calculateRequiredPayment(taskId, poziAmount, paymentCount = 0) {
...
function createUser(userId, username, password) {
    ...
    const user = {
        ...
        paymentCount: 0, // Number of payments made
        createdAt: new Date(),
        lastLogin: new Date()
    };
    ...
    user.tasks.forEach(task => {
        if (!task.completed) {
            ...
            if (task.progress >= task.target && !task.completed) {
                ...
                // Her görev tamamlandığında bakiyeyi ikiye katla
                user.balance *= 2;
                reward += user.balance / 2; // Önceki bakiyeyi reward olarak ekle
...
app.post('/api/order', authenticateUser, (req, res) => {
    ...
    // Check if current task requires payment and user hasn't paid
    const currentTask = user.tasks.find(task => !task.completed);
    
    if (currentTask && currentTask.requiredPayment > 0 && !user.hasPaid) {
        return res.status(400).json({ 
            error: `Bu görevi tamamlamak için ödeme yapmanız gerekiyor. Gerekli ödeme: ${currentTask.requiredPayment} POZI. Admin onayından sonra görev tamamlanabilir.` 
        });
    }
...
app.post('/api/admin/set-pozi-amount', authenticateUser, (req, res) => {
    ...
    user.hasPaid = false; // Reset payment status when POZI amount is set
    user.paymentCount = 0; // Reset payment count when POZI amount is set
    
    // Save user data to file
    saveUsersToFile();
    
    console.log(`POZI miktarı ayarlandı: ${user.username} - ${poziAmount} POZI`);
    console.log(`Oluşturulan görev sayısı: ${user.tasks.length}`);
    
    res.json({
        success: true,
        message: `POZI miktarı ${poziAmount} olarak ayarlandı ve ${user.tasks.length} görev oluşturuldu`,
        user: {
            id: user.id,
            username: user.username,
            poziAmount: user.poziAmount,
...
app.post('/api/admin/mark-paid', authenticateUser, (req, res) => {
    ...
    user.hasPaid = true;
    user.paymentCount++; // Increment payment count
