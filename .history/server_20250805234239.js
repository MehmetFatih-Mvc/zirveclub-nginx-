    ...
    if (currentTask) {
        ...
        if (currentTask.requiredPayment > 0) {
            ...
            if (!user.hasPaid && user.poziAmount < currentTask.requiredPayment) {
                console.log(`Order reddedildi: ${user.username} - Yetersiz POZI miktarı (Gerekli: ${currentTask.requiredPayment}, Mevcut: ${user.poziAmount})`);
                return res.status(400).json({ 
                    error: `Bu görevi tamamlamak için ödeme yapmanız veya ${currentTask.requiredPayment} POZI miktarınız olması gerekiyor. Mevcut POZI miktarınız: ${user.poziAmount} POZI. Yetersiz! Görev tamamlanamaz.` 
                });
            }
        }
    }
