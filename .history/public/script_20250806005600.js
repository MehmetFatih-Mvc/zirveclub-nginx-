...
async function handleRegister() {
    ...
    try {
        ...
        // Eğer response JSON değilse bile hata yakalayalım
        let data;
        try {
...
function updateUI() {
    ...
    // Update stats
    document.getElementById('userBalance').textContent = `${currentUser.balance} Zirve (Kazanılan)`;
    if (currentUser.poziBalance !== undefined) {
        document.getElementById('userBalance').textContent += ` | ${currentUser.poziBalance} Zirve (Zirve Bakiye)`;
    }
    ...
    // Show Zirve amount status
    if (currentUser.poziAmount > 0) {
        document.getElementById('poziAmountStatus').textContent = `${currentUser.poziAmount} Zirve`;
    ...
    if (currentUser.hasPaid) {
        ...
        document.getElementById('paymentStatus').textContent = 'Zirve miktarı belirlenmemiş';
...
function renderTasks() {
    ...
    if (!currentUser.tasks || currentUser.tasks.length === 0) {
        tasksContainer.innerHTML = '<div class="no-tasks">Admin tarafından Zirve miktarı henüz belirlenmemiş.</div>';
        return;
    }
...
function createTaskElement(task) {
    ...
    if (task.requiredPayment > 0) {
        ...
        paymentInfo = `<div class="task-payment">Ödeme Gerekli: ${task.requiredPayment} Zirve</div>`;
    }
    ...
    taskDiv.innerHTML = `
        <div class="task-header">
            ...
            <div class="task-reward">+${reward} Zirve</div>
        </div>
...
function renderWithdrawalsHistory(withdrawals) {
    ...
    if (!withdrawals || withdrawals.length === 0) {
        container.innerHTML = '<div class="no-withdrawals">Henüz para çekme talebi oluşturulmamış</div>';
        return;
    }
    
    container.innerHTML = withdrawals.map(withdrawal => `
        <div class="withdrawal-item ${withdrawal.status}">
            <div class="withdrawal-header">
                <span class="withdrawal-amount">${withdrawal.amount} POZI</span>
                <span class="withdrawal-status">${getWithdrawalStatusText(withdrawal.status)}</span>
            </div>
            <div class="withdrawal-wallet">${withdrawal.walletAddress}</div>
            <div class="withdrawal-date">${new Date(withdrawal.createdAt).toLocaleString('tr-TR')}</div>
            ${withdrawal.processedAt ? `<div class="withdrawal-processed">İşlendi: ${new Date(withdrawal.processedAt).toLocaleString('tr-TR')}</div>` : ''}
            ${withdrawal.status === 'pending' ? '<div class="withdrawal-note">Admin onayı bekleniyor</div>' : ''}
        </div>
    `).join('');
}
