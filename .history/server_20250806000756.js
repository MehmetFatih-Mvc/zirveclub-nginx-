...
function updateTaskProgress(user, actionType) {
    ...
    if (currentTask) {
        ...
        if (currentTask.progress >= currentTask.target && !currentTask.completed) {
            ...
            // Kazanç hesaplama - her görevde bir önceki görevin 0.90 katı
            if (currentTask.id === 1) {
                // İlk görev için sabit miktar
                const firstReward = 10;
                user.balance += firstReward;
                reward += firstReward;
                console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${firstReward} POZI kazandı (İlk görev)`);
            } else {
                // Diğer görevler için bir önceki görevin 0.90 katı
                ...
                if (previousTask && previousTask.completed) {
                    ...
                    const currentReward = previousReward * 0.9;
                    user.balance += currentReward;
                    reward += currentReward;
                    console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${currentReward} POZI kazandı (Önceki: ${previousReward} × 0.9)`);
                } else {
                    // Bir önceki görev tamamlanmamışsa sabit miktar
                    const fallbackReward = 10;
                    user.balance += fallbackReward;
                    reward += fallbackReward;
                    console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${fallbackReward} POZI kazandı (Fallback)`);
                }
            }
