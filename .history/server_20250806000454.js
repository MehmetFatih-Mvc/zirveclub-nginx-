    ...
    if (currentTask) {
        ...
        if (currentTask.progress >= currentTask.target && !currentTask.completed) {
                ...
                if (task.id === 1) {
                    ...
                    reward += firstReward;
                    console.log(`${user.username} kullanıcısı ${task.id}. görev için ${firstReward} POZI kazandı (İlk görev)`);
                } else {
                    // Diğer görevler için bir önceki görevin 0.90 katı
                    const previousTask = user.tasks.find(t => t.id === task.id - 1);
                    if (previousTask && previousTask.completed) {
                        ...
                        console.log(`${user.username} kullanıcısı ${task.id}. görev için ${currentReward} POZI kazandı (Önceki: ${previousReward} × 0.9)`);
                    } else {
                        // Bir önceki görev tamamlanmamışsa sabit miktar
                        const fallbackReward = 10;
                        user.balance += fallbackReward;
                        reward += fallbackReward;
                        console.log(`${user.username} kullanıcısı ${task.id}. görev için ${fallbackReward} POZI kazandı (Fallback)`);
                    }
                }
