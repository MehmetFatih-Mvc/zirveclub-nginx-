...
function generateTasksForUser(poziAmount) {
    return baseTaskDefinitions.map(task => ({
        ...
        hasPaid: false // Her görev için ayrı ödeme durumu
    }));
}
...
function createUser(userId, username, password) {
    ...
    const user = {
        ...
        balance: 0, // Kazanılan para (görev tamamlandıkça artar)
        poziBalance: 0, // POZI bakiyesi (ayrı tutulur)
        totalReceived: 0,
        totalGiven: 0,
        dailyOrders: 0,
        weeklyOrders: 0,
        ...
        poziAmount: 0, // Admin will set this (sabit POZI miktarı)
        hasPaid: false, // Payment status (genel)
        createdAt: new Date(),
        lastLogin: new Date()
    };
...
function updateTaskProgress(user, actionType) {
    ...
    // Her görev için ayrı hasPaid kontrolü yapılacak
    ...
    // Check for new task completions - sadece sıradaki görev tamamlanabilir
    const currentTask = user.tasks.find(task => !task.completed);
    
    if (currentTask) {
        // Sadece sıradaki görev için kontrol yap
        ...
        currentTask.progress = Math.min(currentProgress, currentTask.target);
        
        if (currentTask.progress >= currentTask.target && !currentTask.completed) {
            // Eğer görev ödeme gerektiriyorsa ve kullanıcı ödeme yapmamışsa POZI miktarı kontrol et
            if (currentTask.requiredPayment > 0 && !currentTask.hasPaid) {
                console.log(`Kontrol: ${user.username} - Görev ${currentTask.id} - Gerekli: ${currentTask.requiredPayment}, Mevcut: ${user.poziAmount}`);
                if (user.poziAmount >= currentTask.requiredPayment) {
                    // POZI miktarı yeterliyse ödeme yapmış sayılır
                    currentTask.hasPaid = true;
                    console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${currentTask.requiredPayment} POZI ödeme gereksinimini karşıladı (POZI miktarı: ${user.poziAmount})`);
                } else {
                    // POZI miktarı yetersizse görev tamamlanamaz
                    console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${currentTask.requiredPayment} POZI gereksinimini karşılayamadı (POZI miktarı: ${user.poziAmount})`);
                    return { reward: 0, newCompletions: [], error: `Görev ${currentTask.id} için yetersiz POZI miktarı. Gerekli: ${currentTask.requiredPayment}, Mevcut: ${user.poziAmount}` };
                }
            }
            ...
            // Kazanç hesaplama - kullanıcının yatırdığı POZI miktarının katları
            const poziAmount = user.poziAmount; // Kullanıcının yatırdığı POZI miktarı (örn: 500)
            
            if (currentTask.id === 1) {
                // İlk görev için POZI miktarının 1 katı
                const firstReward = poziAmount;
                user.balance += firstReward;
                reward += firstReward;
                console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${firstReward} POZI kazandı (POZI miktarının 1 katı)`);
            } else {
                // Diğer görevler için POZI miktarının katları
                const previousTask = user.tasks.find(t => t.id === currentTask.id - 1);
                if (previousTask && previousTask.completed) {
                    // Bir önceki görevin kazancını hesapla
                    let previousReward = 0;
                    if (previousTask.id === 1) {
                        previousReward = poziAmount; // İlk görev için POZI miktarı
                    } else {
                        const prevPrevTask = user.tasks.find(t => t.id === previousTask.id - 1);
                        if (prevPrevTask && prevPrevTask.completed) {
                            if (prevPrevTask.id === 1) {
                                previousReward = poziAmount * 9; // İkinci görev için POZI miktarının 9 katı
                            } else {
                                // Daha önceki görevler için recursive hesaplama
                                let baseReward = poziAmount;
                                for (let i = 2; i <= prevPrevTask.id; i++) {
                                    baseReward *= 9;
                                }
                    ...
                    const currentReward = previousReward * 9;
                    user.balance += currentReward;
                    reward += currentReward;
                    console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${currentReward} POZI kazandı (Önceki: ${previousReward} × 9)`);
                } else {
                    // Bir önceki görev tamamlanmamışsa POZI miktarının 1 katı
                    const fallbackReward = poziAmount;
                    user.balance += fallbackReward;
                    reward += fallbackReward;
                    console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için ${fallbackReward} POZI kazandı (Fallback - POZI miktarının 1 katı)`);
                }
            }
            
            newCompletions.push(currentTask);
        }
    }
...
app.use(cors({
    origin: ['https://dijital-menu.shop', 'http://localhost:3000', 'https://www.dijital-menu.shop'],
    credentials: true,
...
app.post('/api/order', authenticateUser, (req, res) => {
    ...
    if (currentTask) {
        console.log(`Order kontrol: ${user.username} - Görev: ${currentTask.id}, Gerekli: ${currentTask.requiredPayment}, Mevcut: ${user.poziAmount}, hasPaid: ${currentTask.hasPaid}`);
        
        // Sıra kontrolü - önceki görevler tamamlanmış mı?
        if (currentTask.id > 1) {
            const previousTask = user.tasks.find(task => task.id === currentTask.id - 1);
            if (previousTask && !previousTask.completed) {
                console.log(`Order reddedildi: ${user.username} - Görev ${currentTask.id} için önceki görev (${previousTask.id}) tamamlanmamış`);
                return res.status(400).json({ 
                    error: `Görev ${currentTask.id} için önceki görev (${previousTask.id}) tamamlanmamış! Görevleri sırayla tamamlamalısınız.` 
                });
            }
        }
        
        // Her görevden önce POZI miktarını kontrol et
        if (currentTask.requiredPayment > 0) {
            // Eğer kullanıcı ödeme yapmamışsa ve POZI miktarı yetersizse görev yapılamaz
            if (!currentTask.hasPaid && user.poziAmount < currentTask.requiredPayment) {
                console.log(`Order reddedildi: ${user.username} - Görev ${currentTask.id} için yetersiz POZI miktarı (Gerekli: ${currentTask.requiredPayment}, Mevcut: ${user.poziAmount})`);
                return res.status(400).json({ 
                    error: `Görev ${currentTask.id} için yetersiz POZI miktarı! Gerekli: ${currentTask.requiredPayment} POZI, Mevcut: ${user.poziAmount} POZI. Bu görev tamamlanamaz.` 
                });
            }
        }
    }
...
app.get('/api/admin/users', authenticateUser, (req, res) => {
    ...
    let usersList = Array.from(users.values()).map(user => ({
        ...
        username: user.username,
        balance: user.balance,
        poziBalance: user.poziBalance || 0,
        totalReceived: user.totalReceived,
        totalGiven: user.totalGiven,
        poziAmount: user.poziAmount,
        hasPaid: user.hasPaid,
...
app.get('/api/withdrawals', authenticateUser, (req, res) => {
    const userWithdrawals = withdrawalRequests
        .filter(withdrawal => withdrawal.userId === req.user.id)
        .map(withdrawal => ({
            id: withdrawal.id,
...
app.post('/api/admin/mark-paid', authenticateUser, (req, res) => {
    ...
    // Mevcut görev için hasPaid'i true yap
    const currentTask = user.tasks.find(task => !task.completed);
    if (currentTask && currentTask.requiredPayment > 0) {
        currentTask.hasPaid = true;
        console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için admin tarafından ödeme yapmış olarak işaretlendi`);
    }
    ...
    res.json({
        ...
        user: {
            id: user.id,
            username: user.username,
            currentTask: currentTask ? { id: currentTask.id, hasPaid: currentTask.hasPaid } : null
        }
    });
});
...
app.post('/api/admin/receipt/:receiptId', authenticateUser, (req, res) => {
    ...
    if (action === 'approve') {
        ...
        if (user) {
            ...
            if (currentTask && currentTask.requiredPayment > 0) {
                currentTask.hasPaid = true;
                console.log(`${user.username} kullanıcısı ${currentTask.id}. görev için dekont onaylandı`);
            }
            // Save user data to file
            saveUsersToFile();
        }
...
// Global BTC wallet address (admin tarafından güncellenebilir)
let btcWalletAddress = 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh';
...
// Update BTC wallet address (admin)
app.post('/api/admin/update-btc-address', authenticateUser, (req, res) => {
    if (req.user.role !== 'admin') {
        return res.status(403).json({ error: 'Yetkisiz erişim' });
    }
    ...
    if (!address) {
        return res.status(400).json({ error: 'BTC adresi gerekli' });
    }
    
    btcWalletAddress = address;
    
    console.log(`BTC adresi güncellendi: ${address}`);
    
    res.json({
        success: true,
        message: 'BTC adresi güncellendi',
        address: btcWalletAddress
    });
});
...
const DOMAIN = process.env.DOMAIN || 'https://dijital-menu.shop';
server.listen(PORT, () => {
    console.log(`ZirveClub Sistemi ${PORT} portunda çalışıyor`);
