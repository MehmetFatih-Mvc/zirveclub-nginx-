    ...
    if (currentTask && currentTask.requiredPayment > 0) {
        ...
        if (!user.hasPaid && user.poziAmount < currentTask.requiredPayment) {
            return res.status(400).json({ 
                error: `Bu görevi tamamlamak için ödeme yapmanız veya ${currentTask.requiredPayment} POZI miktarınız olması gerekiyor. Mevcut POZI miktarınız: ${user.poziAmount} POZI` 
            });
        }
    }
